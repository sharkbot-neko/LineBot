...
def run(ctx: Context, args):
    if ctx.sender == ctx.owner_mid:
        if not args:
            ctx.line.sendMessage(ctx.receiver, '警告リセットの使い方\n!reset_warn [mid]')
            return
        else:
            target = " ".join(args)
        try:
        ...
        try:
            ...
            Warns(ctx.cur, ctx.conn).reset_warn(ctx.receiver, target_contact.mid)
            ctx.line.sendMessage(ctx.receiver, f"{display_name}の警告回数をリセットしました。")
        except:
            return